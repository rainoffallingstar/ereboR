...
#' @return list
    download = function(){
      ...
      if (self$Dwarf_worker == "TGCA"){
        ...
        fs::dir_create('data/')
        data <- GEOquery::getGEO(self$mine,destdir = 'data/')
        return(data)
      }else {
...
MethylEntClass <- R6::R6Class(
  ...
  public = list(
    DMP_table <- NA,
    initialize <- function(DMP_table = NA){
      self$DMP_table <- self$DifferentAnalysis_workfolw(adjust.method = "BH",adjPVal = 0.05)
    },
    
    removeBatch = function( variablename = "group",
                            batchname="batch",
                            logitTrans=TRUE){
      ...
      if ("data.frame" %in% class(beta)){
        print("we repuire a methylationSet in the format of data.frame as input")
      }
      if ("data.frame" %in% class(pd)){
        print("we repuire a pd in the format of data.frame as input")
      }
      ...
      if(ncol(PhenoTypes.lv)>=1){
        ...
        message("[Combat have automatically select ALL factors contain at least two different values from your pd(sample_sheet.csv).]")
      }else{
      ...
      innercombat <- function(beta,beta_2,batch_temp,formdf,pd,logitTrans){
        ...
        if (is.na(combat[1,1])){
          ...
          combat <- sva::ComBat(dat = beta_2[,-1],batch=batch_temp,mod = mod)
        }
        return(combat)
      }
      library(sva)
    ...
    probel_annotate = function(type = "cpg",cpg_bedgraph){
      ...
      rrbs_annote4DMP <- function(DMP_df,type = "cpg",
        ...
        if (type == "promoter"){
          idx_temp <- idx$idx1
          for (i in 1:length(idx_temp)) {
            temp <- stringr::str_split(idx_temp[i],"_")[[1]][2]
            idx_temp[i] <- stringr::str_sub(temp,1,nchar(temp)-1)
          }
          idx <- idx %>% mutate(label = idx_temp)
        } else {
      ...
      df <- rrbs_annote4DMP(self$DMP_table,type,cpg_bedgraph)
      return(df)
    },
    gene_enrich = function(changeType,pathwaydb,genedb){
      enrichment_in_action <- function(df,types){
        if (types == "go"){
          result <- clusterProfiler::enrichGO(df, 'org.Hs.eg.db', ont="BP", pvalueCutoff=0.01)
        } else {
      ...
      gene2entrez <- function(genelist,db){
        # need further tidy evaluation
        db %>% 
          dplyr::filter(SYMBOL %in% genelist) %>% 
          dplyr::select(ENTREZID) %>% 
    ...
    #dashboard = function(){}
    Mvalue_transform = function(normalize = FALSE){
      matrixdf <- self$array[,-1] %>% as.matrix()
