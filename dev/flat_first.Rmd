...
#' @param save2bismark default as F,generate a bismark_cov bedgraph file if True
#' @param use_conda_env use a conda env if not NULL
#' @param cores cpu cores to be used
#' @param mem memory to be used
#'
#' @return methyratio
bam2methyratio = function(save2bismark = F,use_conda_env = NULL,cores = 16, mem = 100000) {
    ...
    for (i in 1:length(bamfiles)) {
      ...
      command <- paste("python2", self$methyratio_py, "-I -o", output_dir_txt,"-M",mem,"-N",cores, "-d", self$gene_fa, paste0(self$root_dir,
        "/", bamfiles[i]))
      if (!is.null(use_conda_env)){
        command <- paste("conda run -n",use_conda_env,command)
      }
      ...
      if (save2bismark){
        bismark_output <- stringr::str_replace(output_dir_bed,".bedgraph",".bismark_cov")
        bsmapz_data <- data.table::fread(input_file, sep = "\t", header = TRUE)
        # 转换为 Bismark Coverage 格式，pos 需要减 1
        bismark_cov <- bsmapz_data[, .(
          chr,                # 染色体名称
          start = pos - 1,    # pos 转换为 0-based 坐标
          methylation_ratio = round(ratio * 100, 2),  # 转为百分比并保留两位小数
          methylated = C_counts,                     # 被甲基化的读段数量
          unmethylated = CT_counts - C_counts        # 非甲基化读段数量
          )]
        # 写入输出文件
        data.table::fwrite(bismark_cov, bismark_output, sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
      }
